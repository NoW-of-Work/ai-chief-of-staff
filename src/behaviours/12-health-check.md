...
1. `{{MANUAL}}`. Read it for the gates, and read section 1 for bracketed fields.
...
Check `{{MANUAL}}` as well. `[LEADER NAME]`, `[TITLE]`, or `[ORGANIZATION]` still sitting in section 1 means the mandate was never written for anyone in particular.
...
Contradictions between files, capped at three. More than three and the leader reads none of them. A contradiction is not proof of an error. Say which file you believe and say why. `commitments.md` outranks a transcript, and a meeting file outranks a person file, per `{{MANUAL}}` section 5.
...
- May propose an exact edit to `about-me.md`, `my-work.md`, or `{{MANUAL}}`, and write it if the leader says yes in the same session.
- May **not** write that edit on a scheduled run, or on any run where the leader is not there to answer. Filling a bracketed field is a decision nobody has made yet. Priorities are `{{MANUAL}}` gate 7.
