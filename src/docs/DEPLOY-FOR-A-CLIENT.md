...
{{/claude}}
{{#chatgpt}}
On ChatGPT the home is a connected app such as Google Drive, and "cloud storage the leader owns" is the right phrase. If the organization blocks personal cloud storage, use the corporate one. What matters is that a scheduled task can reach it and the leader can open it on a phone.
