...
**It reads. It does not close.** Promoting an entry to `Made` needs the leader saying what they decided, in their own words, in that session. That is gate 11 in `{{MANUAL}}`, and a scheduled run has nobody in the room.
