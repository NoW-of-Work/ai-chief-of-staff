> **health-check**. Audit whether the workspace is actually configured: blocking gaps, stale files, contradictions between files, output folders that are not filling, and one honest verdict. 350 words. Monthly.
>
...
> "run health-check" and point at the file. The project instructions
...
1. `PROJECT-INSTRUCTIONS.md`. Read it for the gates, and read section 1 for bracketed fields.
...
Check `PROJECT-INSTRUCTIONS.md` as well. `[LEADER NAME]`, `[TITLE]`, or `[ORGANIZATION]` still sitting in section 1 means the mandate was never written for anyone in particular.
...
ChatGPT cannot list a folder. Ask once for the date on the newest file in `briefs/` whose name ends `-brief.md`, ignoring closes, triage passes, reviews and audits, and whether `meetings/` and `people/` have anything in them. One question, and take whatever answer comes back.
...
Contradictions between files, capped at three. More than three and the leader reads none of them. A contradiction is not proof of an error. Say which file you believe and say why. `commitments.md` outranks a transcript, and a meeting file outranks a person file, per `PROJECT-INSTRUCTIONS.md` section 5.
...
- May propose an exact edit to `about-me.md`, `my-work.md`, or `PROJECT-INSTRUCTIONS.md`, and write it if the leader says yes in the same session.
- May **not** write that edit on a scheduled run, or on any run where the leader is not there to answer. Filling a bracketed field is a decision nobody has made yet. Priorities are `PROJECT-INSTRUCTIONS.md` gate 7.
