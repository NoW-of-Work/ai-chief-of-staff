> **onboarding**. Set up the AI Chief of Staff. Creates any missing workspace files, detects connected tools, drafts about-me.md and my-work.md from real sources, then asks only what no tool can reveal.
>
...
> "run onboarding" and point at the file. The project instructions
...
| Path | If missing |
|------|-----------|
| `PROJECT-INSTRUCTIONS.md` | Write it out from the copy in the project files |
| `about-me.md` | Write out the blank template |
| `my-work.md` | Write out the blank template |
| `tomorrow.md` | Write out the blank template |
| `commitments.md` | Write out the blank template |
| `decisions.md` | Write out the blank template |
| `connections.md` | Write out the blank template |
...
ChatGPT cannot write files to a folder on its own. Produce the missing files as clearly labelled code blocks and tell the leader to save each one into the project. One message, one file per block, filenames as headings. Do not continue to Step 1 until the leader confirms the files exist.
...
people/, archive/.
Already there: PROJECT-INSTRUCTIONS.md, about-me.md, my-work.md, commitments.md.
```
...
ChatGPT cannot enumerate its own connectors. Ask once, in a single message, which of the seven capabilities the leader has: Calendar, Email, Documents, Transcripts, Chat, Tasks, CRM.

Then confirm each claimed one by trying to read from it. A tool the leader says is connected but that returns nothing is `failing`, not `connected`, and that distinction is worth more than their answer.

Anything the leader cannot connect is still workable. They can paste a calendar, a thread, or a transcript into the chat and every behaviour will use it. Record the capability as `missing` anyway, because the registry is about what the system can reach on its own.
...
Output the completed `about-me.md`, `my-work.md`, and `connections.md` as three code blocks for the leader to save over the blanks. Keep the `(inferred)` labels in the files. They are how the system knows what it is still standing on a guess about.
