> **inbox-triage**. Mail since the last pass, cut to what needs a person: decisions, drafted replies, promises in both directions, and a count of what was left alone. 350 words. Use when the inbox has run ahead.
>
...
> "run inbox-triage" and point at the file. The project instructions
...
Append each to `commitments.md` as an `[Active]` entry, direction `By me`, with the mail named in the source field. A promise the mail does not clearly support gets written as `[To verify]` instead, per `PROJECT-INSTRUCTIONS.md` gate 4, and gets one line here so the leader can settle it.
...
- **Never change the state of a message.** `PROJECT-INSTRUCTIONS.md` gate 12 covers what that means and why.
...
- May **not** open a message in a way that changes its state in the mail client. Read-only, always, per `PROJECT-INSTRUCTIONS.md` gate 12.
- May not send, reply, forward, or schedule a send, ever, for any draft in this pass. That is `PROJECT-INSTRUCTIONS.md` gate 1 and no draft is exempt from it.
