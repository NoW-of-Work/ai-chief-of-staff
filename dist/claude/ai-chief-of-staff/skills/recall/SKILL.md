...
Search in this order. Deference, when two of these disagree, is `CLAUDE.md` section 5 and it is not the same order.
...
Deference is `CLAUDE.md` section 5, all seven rules, read there and not restated here. This behaviour extends it in one place: how `decisions.md` statuses rank inside rule 3. A `Made` or `Stood` entry beats the meeting file the decision came from. A `To verify` entry beats nothing, and renders as `(uncertain)`.
...
- May append a commitment to `commitments.md` when the search turns up a promise with a clear source, per `CLAUDE.md` gate 4.
...
- May not write personal or sensitive detail found during a search into `people/`. This is `CLAUDE.md` gate 6.
